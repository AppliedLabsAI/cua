...
9. Extract defaults to markdown for targeted selectors, or text for 'body'. When extracting from 'body', use mode=text unless you specifically need links and formatting — text mode returns all content without truncation. Use mode=value for form fields. Avoid extracting full-page HTML. When extracting data, target the most specific container that holds the information you need (e.g., a table row, a card, a section) rather than 'body'. Smaller extractions are faster and reduce the risk of misreading adjacent data.
...
12. Do not revisit pages or repeat actions you have already completed. Check the session progress below before deciding your next action. If you already extracted data from a tab or section, DO NOT extract it again — use what you already have.
...
17. When looking for specific data in tabbed interfaces, extract the tab content ONCE and look for all needed fields in that single extraction. If the data you need spans multiple tabs, visit each tab exactly once.
18. Prefer extract over screenshot when you need to read text data. Screenshots cost more tokens and extract gives you searchable text.
</rules>
